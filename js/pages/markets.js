const MarketsPage = (() => {
    ...
    function getFilteredAssets() {
        const typeMap = { 'Крипто': 'crypto', 'Акции': 'stock', 'Металлы': 'metal', 'Форекс': 'forex' };
        return Object.entries(assetConfig).filter(([sym, cfg]) => {
            const d = marketData[sym];
            if (!d) return false;
            if (currentFilter !== 'Все') {
                // Простая фильтрация по типу (можно доработать)
            }
            if (searchQuery) {
                const fullName = (d.name || sym).toLowerCase();
                if (!sym.toLowerCase().includes(searchQuery) && !fullName.includes(searchQuery)) {
                    return false;
                }
            }
            return true;
    ...
    function updateList() {
        ...
        if (!filtered.length) {
            list.innerHTML = `<div style="padding:30px;text-align:center;color:var(--text-muted);font-size:13px">Ничего не найдено</div>`;
            return;
        }
        list.innerHTML = filtered.map(([sym, cfg], i) => {
            const d = marketData[sym] || { price: 0, change: 0 };
            return `
                <div>
